...
const Player = class Player {
  constructor(PlayerId) {
    ...
    this.discord = {
      id: this.server.identifiers.find((identifier) =>
        identifier.includes("discord:")
      )
        ? this.server.identifiers
            .find((identifier) => identifier.includes("discord:"))
            .split(":")[1]
        : null,
      guilds: [],
    };
    this.cache = {}; // extensions and different resources can store things here and use it
  ...
  async getDatabase(filter = {}) {
    if (
      ...
      throw new Error(
        "DiscordFramework: Player --> getInfractions() filter must be an Object"
      );

    const {
      helpers: { FindOne },
    } = require("../core/index").MongoDB;
  ...
  #UpdateDatabaseInformation() {
    ...
    const {
      helpers: { FindOne, InsertOne, UpdateOne },
    } = require("../core/index").MongoDB;

    // Database
    FindOne(
      "Players",
      { "information.identifiers": { $in: this.server.identifiers } },
      async (result) => {
        if (result) {
          ...
          const NewIdentifiers = this.server.identifiers.filter(
            (identifier) => !result.information.identifiers.includes(identifier)
          );
          ...
          let GeoIP = await fetch(
            "http://ip-api.com/json/" +
              this.server.identifiers
                .find((iden) => iden.includes("ip:"))
                .split(":")[1]
          );
          ...
          if (GeoIP.status.toLowerCase() === "success") {
            ...
            ] = `${GeoIP.country}, ${GeoIP.regionName}, ${GeoIP.city}`;
          }
  ...
  #DiscordGuildsUpdate() {
    ...
    this.discord.guilds = Discord.helpers
      .GetSharedGuilds(this.discord.id)
      .map((guild) => {
    ...
    Discord.client.on("guildMemberUpdate", (oldM, newM) => {
      if (
        this.discord.id &&
        newM.id === this.discord.id &&
        !this.server.connections.disconnectedAt
      ) {
        Debug(
          `(${this.getServerId()}) ${this.getName()}'s roles were updated!`
        );
        const Guild = this.discord.guilds.find(
          (guild) => guild.id === newM.guild.id
        );
        if (Guild) {
          Guild.name = newM.guild.name;
          Guild.administrator = newM.permissions.has("ADMINISTRATOR");
          Guild.roles = newM.roles.cache.map((role) => ({
            id: role.id,
            name: role.name,
          }));
        } else {
          this.discord.guilds.push({
            ...
            roles: newM.roles.cache.map((role) => ({
              id: role.id,
              name: role.name,
            })),
          });
...
const PlayerSet = class PlayerSet extends Set {
  ...
  get(Identifier) {
    ...
    if (!Identifier)
      return new Error(
        "Players.prototype.get(Identifier) ---> Unknown Identifier"
      );
    if (typeof Identifier === "object" && Array.isArray(Identifier)) {
      ...
      if (ValIden === "Server") {
        ...
        const _Player = _Players
          .filter((player) => String(player.server.id) === Identifier)
          .sort((a, b) => b.server.id - a.server.id)[0];
        if (_Player) {
          return _Player;
        } else {
          return undefined;
        ...
        const _Players = this.toArray();
        const _Player = _Players
          .filter((player) => String(player.discord.id) === Identifier)
          .sort((a, b) => b.server.id - a.server.id)[0];
        if (_Player) {
          return _Player;
        } else {
        ...
        const _Player = _Players
          .filter((player) =>
            player.server.identifiers
              .map((iden) => String(iden).toLowerCase())
              .find((iden) => iden === Identifier)
          )
          .sort((a, b) => b.server.id - a.server.id)[0];
        if (_Player) {
          return _Player;
    ...
    if (typeof filters !== "object" && !Array.isArray(filters)) {
      if (typeof filters === "string") {
        ...
        return console.error(
          "PlayerSet.filter(filters) Filters parameter must be an array!"
        );
      }
    }
  ...
  validateIdentifier(Identifier) {
    if (
      (Identifier.includes("license:") && typeof Identifier === "string") ||
