...
on("DiscordFramework:Core:Discord:Ready", async () => {
  ...
  const Directory =
    GetResourcePath(GetCurrentResourceName()) + "/src/extensions";
  ...
  let counter = 0;
  while (
    Extensions.toArray().length !==
    LocalExtensionsFile.filter((f) => f.files.length > 0).length
  ) {
    if (counter > 20) {
      break;
    }
  ...
  if (
    ...
    LocalExtensionsFile.filter(
      (lef) =>
        !Extensions.toArray().find(
          (e) => lef.name.toLowerCase() === e.name.toLowerCase()
        )
    ).forEach((lef) => {
      new Extension({ name: lef.name }).setStatus("Unknown");
    });
  }
 ...
 */
const GetInfo = () => {
  const ReadyExtensions = Extensions.toArray().filter(
    (e) => e.status === "Ready"
  );
  const EnabledExtensions = Extensions.toArray().filter(
    (e) => e.status === "Enabled"
  );
  const NoneEnabledExtensions = Extensions.toArray().filter(
    (e) =>
      e.status !== "Ready" && e.status !== "Enabled" && e.status !== "Template"
  );
  const TemplateExtensions = Extensions.toArray().filter(
    (e) => e.status === "Template"
  );
  ...
  const SortedExtensions = [
    ...
    const result = {};
    result["^3Index"] =
      (extension.status === "Template" ? "^6" : color) + index;
    result["^3Name"] =
      (extension.status === "Template" ? "^6" : color) + extension.name;
    result["^3Description"] =
      (extension.status === "Template" ? "^6" : color) + extension.description;
    result["^3Status"] =
      extension.status === "Template"
        ? "^6" + extension.status
        : extension.status !== "Enabled"
        ? "^8" + extension.status
        : "^2" + extension.status;
    result["^3Author"] =
      (extension.status === "Template" ? "^6" : color) +
      (extension.author ? extension.author : "Unknown");
    result["^3Version"] =
      (extension.status === "Template" ? "^6" : color) +
      (extension.version ? extension.version : "Unknown");
    return result;
  });
