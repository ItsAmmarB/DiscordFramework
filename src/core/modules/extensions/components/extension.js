...
class Extension {
  ...
  constructor({
    ...
    if (Extension.prototype.constructor !== this.constructor)
      console.log(
        `The Extension class was meant to be invoked as a new instance for each extension; using it as a parent class for an extended class for ${
          name ? 'the "' + name + '"' : "an"
        } extension will lose some of its useful methods/methods' parameters such as Extension.setFunction() parameters and Extension.setCFXExports()`
      );

    // Check Extension name
    if (!name)
      return this.#flagError(
        "REG_EXTENSION: attempted to register an extension without a name!"
      );
    if (typeof name !== "string")
      this.#flagError(
        `REG_EXTENSION: Extension "name" type must be "String", received "${typeof name}"!`
      );
    this.name = name;
    ...
    if (description) {
      if (typeof description === "string") {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "description" type must be "String", received "${typeof description}"!`
        );
      }
    }
    ...
    if (typeof toggle !== typeof undefined) {
      if (typeof toggle === "boolean") {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "Toggle" type must be "Boolean"; received "${typeof toggle}"!`
        );
      }
    }
    ...
    if (dependencies) {
      if (Array.isArray(dependencies)) {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "Dependencies" type must be "Array", received "${typeof dependencies}"!`
        );
      }
    }
    ...
    if (author) {
      if (typeof author === "string") {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "Author" type must be "String", received "${typeof author}"!`
        );
      }
    }
    ...
    if (version) {
      if (typeof version === "string") {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "Version" type must be "String", received "${typeof version}"!`
        );
      }
    }
    ...
    if (config) {
      if (typeof config === "object" && !Array.isArray(config)) {
        ...
        this.#flagError(
          `REG_EXTENSION: Extension "Config" type must be "Object", received "${typeof config}"!`
        );
      }
    }

    if (Extensions.get(this.name))
      return this.#flagError(
        `REG_EXTENSION: Attempting to register an extension with a duplicate name "${this.name}"!`
      );
  ...
  async #register() {
    ...
    if (this.dependencies.length > 0) {
      ...
      if (dependencies.find((d) => d.status === "Template")) {
        ...
        console.warn(
          `The Extensions Template cannot be a dependency for an extension; Dependency was ignored in the "${this.name}" extension`
        );
      }

      if (dependencies.find((d) => d.status === "Self-Dependency")) {
        dependencies = dependencies.filter(
          (d) => d.status !== "Self-Dependency"
        );
        console.warn(
          `An extension cannot be a dependency for itself; Dependency was ignored in the "${this.name}" extension`
        );
      }

      const disabledDependency = dependencies.find(
        (d) => d.status === "Disabled"
      );
  ...
  runClient(playerId = -1, payload = {}) {
    ...
    if (playerId) {
      if (isNaN(playerId))
        return this.#flagError(
          `Extension<${this.name}>.executeClient(playerId, ...) playerId type must be "Number"!`
        );
    }
    emitNet(
      "DiscordFramework:Extensions:RunClient:" + this.name,
      playerId,
      payload
    );
    return this;
  ...
  setCFXExports(Exports) {
    ...
    if (typeof Exports !== "object")
      return this.#flagError(
        `Extension<${
          this.name
        }>.setCFXExports() Parameter type must be "Object"; received "${typeof Exports}"!`
      );
    if (Array.isArray(Object))
      return this.#flagError(
        `Extension<${this.name}>.setCFXExports() Parameter type must be "Object"; received "Array"!`
      );
