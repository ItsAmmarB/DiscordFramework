...
const GetGuild = async (GuildId) => {
  if (!GuildId)
    throw new Error(
      "DiscordFramework: Discord --> GetGuild() No guild ID provided"
    );
  if (!IsSnowFlake(GuildId))
    throw new Error(
      "DiscordFramework: Discord --> GetGuild() Invalid guild ID provided"
    );
...
const GetSharedGuilds = (UserId) => {
  if (!UserId)
    throw new Error(
      "DiscordFramework: Discord --> GetSharedGuilds() No player ID provided"
    );
  if (!IsSnowFlake(UserId))
    throw new Error(
      "DiscordFramework: Discord --> GetSharedGuilds() Invalid player ID provided"
    );
  ...
  if (NetworkPlayers.validateIdentifier(UserId) !== "Discord") {
    ...
    if (!Player)
      throw new Error(
        "DiscordFramework: Discord --> GetSharedGuilds() Player does not have a PlayerNetworkObject"
      );
...
const GetRole = async (
  RoleId,
  GuildId = Config.core.discord.communityGuild.id
) => {
  if (!RoleId)
    throw new Error(
      "DiscordFramework: Discord --> GetRole() No role ID provided"
    );
  if (!IsSnowFlake(RoleId))
    throw new Error(
      "DiscordFramework: Discord --> GetRole() Invalid role ID provided"
    );

  if (!IsSnowFlake(GuildId))
    throw new Error(
      "DiscordFramework: Discord --> GetRole() Invalid guild ID provided"
    );
  const Guild = await GetGuild(GuildId);
  if (!Guild)
    throw new Error(
      "DiscordFramework: Discord --> GetRole() Client is not a member of provided guild's ID or guild does not exist"
    );
...
const GetMember = async (
  ...
  if (!UserId)
    throw new Error(
      "DiscordFramework: Discord --> GetMember() No player ID provided"
    );
  ...
  if (NetworkPlayers.validateIdentifier(UserId) !== "Discord") {
    ...
    if (!Player)
      throw new Error(
        "DiscordFramework: Discord --> GetMember() Player does not have a PlayerNetworkObject"
      );
    UserId = Player.discordId;
  }
  ...
  if (!IsSnowFlake(UserId))
    throw new Error(
      "DiscordFramework: Discord --> GetMember() Invalid player ID provided"
    );

  const Guild = await GetGuild(GuildId);
  if (!Guild)
    throw new Error(
      "DiscordFramework: Discord --> GetMember() Client is not a member of provided guild's ID or guild does not exist"
    );
...
const GetUser = async (UserId) => {
  if (!UserId)
    throw new Error(
      "DiscordFramework: Discord --> GetUser() No player ID provided"
    );
  ...
  if (NetworkPlayers.validateIdentifier(UserId) !== "Discord") {
    ...
    if (!Player)
      throw new Error(
        "DiscordFramework: Discord --> GetUser() Player does not have a PlayerNetworkObject"
      );
    UserId = Player.discordId;
  }
  ...
  if (!IsSnowFlake(UserId))
    throw new Error(
      "DiscordFramework: Discord --> GetUser() Invalid player ID provided"
    );
...
const IsSnowFlake = (SnowFlake) => {
  ...
  if (SnowFlakeTS < MixSnowFlakeTS || SnowFlakeTS > MaxSnowFlakeTS)
    return false;
  return true;
};
...
const GetInfo = () => {
  return (
    `\n              ^3Client Details: ^4${
      Client.user.tag + " ^6(" + Client.user.id + ")"
    }` +
    `\n              ^3Client Invite: ^4${Client.generateInvite({
      scopes: ["bot"],
    })}` +
    `\n              ^3Discord Users: ^4${
    ...
    `\n              ^3Discord Guilds: \n${Client.guilds.cache
      .map((guild) => `                ^3- ^4${guild.name} ^6(${guild.id})`)
      .join("\n")}` +
    "\n^0"
  );
};
