...
Permissions.setFunction((ExtensionInfo, { Discord, MongoDB, Utilities }) => {
    ...
    on('DiscordFrameworK:Player:Roles:Updated', player => {
        emitNet('DiscordFrameworK:Extension:Permission:UpdateRole', player.server.id, player);
    });
    ...
    const CheckPermission = (PlayerId, Roles, GuildId) => {
        ...
        if(!Utilities.Players.NetworkPlayers.validateIdentifier(PlayerId)) return false; // Check whether the provided PlayerId is any kind of an identification method
        ...
        const Guilds = GuildId ? [Player.discord.guilds.find(guild => guild.id === GuildId)] : Player.discord.guilds; // Makes the guild/guilds into an altered array for a unified logic
        ...
        for (const Permission of Config.acePermissions.permissions) {
            ...
            if(Permission.groups.length > 0) {
                for (const Group of Permission.groups) {
                    ExecuteCommand(`add_principal identifier.discord:${Player.getDiscordId()} ${Group}`);
                }
            }
            if(Permission.aces.length > 0) {
                for (const Ace of Permission.aces) {
                    ExecuteCommand(`add_ace identifier.discord:${Player.getDiscordId()} ${Ace} allow`);
                }
            }
        }
    };
    ...
    const AceRemove = Player => {
        ...
        for (const Permission of Config.acePermissions.permissions) {
            ...
            if(Permission.groups.length > 0) {
                for (const Group of Permission.groups) {
                    ExecuteCommand(`remove_principal identifier.discord:${Player.getDiscordId()} ${Group}`);
                }
            }
            if(Permission.aces.length > 0) {
                for (const Ace of Permission.aces) {
                    ExecuteCommand(`remove_ace identifier.discord:${Player.getDiscordId()} ${Ace}`);
                }
            }
        }
    };
