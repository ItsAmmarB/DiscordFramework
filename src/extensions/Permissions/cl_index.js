onNet(
  "DiscordFramework:Extensions:RunClient:Permissions",
  ({ player, config }) => {
    console.log(
      "^2[^0Extensions^2:^0Permissions^2]^6: ^3Client script initializing!"
    );
    ...
    const CheckPermission = (Roles, GuildId) => {
      ...
      const Guilds = GuildId
        ? Player.discord.guilds.filter((guild) => guild.id === GuildId)
      ...
      if (
        Roles.find((roleA) =>
          Guilds.find((guild) =>
            guild.roles.find((roleB) => roleA === roleB.id)
          )
        )
      )
