...
require("./version")(async (result) => {
  if (result) {
    if (!result.upToDate) {
      console.log(
        `^3[DiscordFramework] A newer version '${result.remote}' was released! currently running ${result.local}^0`
      );
    } else {
      console.log(
        `^2[DiscordFramework] Running the latest version! ${result.local}^0`
      );
    }
  }
    ...
    if (isReady) {
      ...
      setInterval(() => {
        UpdateMany(
          ...
          {
            _id: {
              $in: NetworkPlayers.filter(["Joined", "!Disconnected"]).map(
                (p) => p.PUID
              ),
            },
          },
...
on("playerConnecting", async (playerName, setKickReason, deferrals) => {
  ...
  const cPlayer = new Player(cPlayerId).setConnectingAt(Date.now());
  ...
  await delay(250);
  let cPlayerOutStandingBans = await cPlayerDatabase.infractions
    .filter((infraction) => infraction.type === "Ban")
    .filter(
      (infraction) =>
        infraction.details.timestamp + infraction.details.duration >=
          Date.now() || infraction.details.duration === 0
    );
  ...
  if (cPlayerOutStandingBans.length > 0) {
     ...
     */
    cPlayerOutStandingBans = cPlayerOutStandingBans.map((ban) => ({
      type: "Container",
      items: [
        {
          type: "TextBlock",
        ...
        {
          type: "TextBlock",
          text: `**Reason**: ${ban.details.reason}`,
          wrap: true,
          fontType: "default",
          size: "Default",
          weight: "Default",
        ...
        },
        {
          type: "TextBlock",
          text: `**Duration**: ${
            ban.details.duration > 0
              ? `${ms(
                  ban.details.timestamp + ban.details.duration - Date.now(),
                  { long: true }
                )} remining`
              : "Indefinite"
          }`,
      ...
      actions: [
        {
          type: "Action.OpenUrl",
          title: "Need Help?",
          iconUrl: "https://cdn-icons-png.flaticon.com/128/1660/1660114.png",
          url: Config.core.discord.communityGuild.invite,
        },
      ],
    });
  }

  if (!cPlayer.getDiscordId())
    return deferrals.done("Discord ID could be detected!");

  if (Config.connection.requireMember.enabled) {
    deferrals.update(`Checking community membership${dots}`);

    const Member = await GetMember(
      cPlayer.getDiscordId(),
      Config.core.discord.communityGuild.id
    );
  ...
  console.log(
    `^9 ===> ^0${cPlayerId} ^9| ^0${cPlayer.getName()} ^9is connecting^0`
  );
  Logger.player(
    `[PUID: ${
      cPlayer.PUID
    }] ${cPlayer.getServerId()} | ${cPlayer.getName()} is connecting`
  );
...
on("playerJoining", async (TempID) => {
  ...
  if (!NetworkPlayers.get(jPlayer.getServerId())) {
    // the only way this could happen, is if the resource were to restart while the player is still joining (in the loadingscreen)
    jPlayer.pushToNetwork();
    await delay(300);
  }

  console.log(
    `^4 ===> ^0${jPlayer.getServerId()} ^4| ^0${jPlayer.getName()} ^4is joining^0`
  );
  Logger.player(
    `[PUID: ${
      jPlayer.PUID
    }] ${jPlayer.getServerId()} | ${jPlayer.getName()} is joining`
  );
...
onNet("playerJoined", async (PlayerId) => {
  ...
  const jPlayer = new Player(PlayerId).setJoinedAt(Date.now());

  await delay(1000);

  if (!NetworkPlayers.get(jPlayer.getServerId())) {
    // the only way this could happen, is if the resource were to restart while the player is in the server
    jPlayer.pushToNetwork();
    await delay(300);
  }
  ...
  console.log(
    `^2 ===> ^0${jPlayer.getServerId()} ^2| ^0${jPlayer.getName()} ^2joined^0`
  );
  Logger.player(
    `[PUID: ${
      jPlayer.PUID
    }] ${jPlayer.getServerId()} | ${jPlayer.getName()} is joined`
  );
});
...
on("playerDropped", async (Reason) => {
  ...
  console.log(
    `^1 ===> ^0${dPlayer.getServerId()} ^1| ^0${dPlayer.getName()} ^1 ${
      Reason === "Exiting" ? "left" : Reason
    }^0`
  );
  Logger.player(
    `[PUID: ${dPlayer.PUID}] ${dPlayer.getServerId()} | ${dPlayer.getName()} ${
      Reason === "Exiting" ? "left" : Reason
    }`
  );
});
